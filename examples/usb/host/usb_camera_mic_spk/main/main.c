...
#define ENABLE_UVC_FRAME_RESOLUTION_ANY                                        \
  0                            /* Using any resolution found from the camera */
...
#else
#define DEMO_UVC_XFER_BUFFER_SIZE (85 * 1024)
#endif
...
static void camera_frame_cb(uvc_frame_t *frame, void *ptr) {
  ESP_LOGI(TAG,
           "uvc callback! frame_format = %d, seq = %" PRIu32
           ", width = %" PRIu32 ", height = %" PRIu32 ", length = %u, ptr = %d",
...
static void mic_frame_cb(mic_frame_t *frame, void *ptr) {
  ...
  ESP_LOGD(TAG,
           "mic callback! bit_resolution = %u, samples_frequence = %" PRIu32
           ", data_bytes = %" PRIu32,
...
static void stream_state_changed_cb(usb_stream_state_t event, void *arg) {
  ...
  case STREAM_CONNECTED: {
    ...
    if (frame_size) {
      ESP_LOGI(TAG, "UAC MIC: get frame list size = %u, current = %u",
               frame_size, frame_index);
      uac_frame_size_t *mic_frame_list =
      ...
      for (size_t i = 0; i < frame_size; i++) {
        ESP_LOGI(TAG,
                 "\t [%u] ch_num = %u, bit_resolution = %u, samples_frequence "
                 "= %" PRIu32 ", samples_frequence_min = %" PRIu32
                 ", samples_frequence_max = %" PRIu32,
                 i, mic_frame_list[i].ch_num, mic_frame_list[i].bit_resolution,
                 mic_frame_list[i].samples_frequence,
                 mic_frame_list[i].samples_frequence_min,
                 mic_frame_list[i].samples_frequence_max);
      }
      ...
      ESP_LOGI(TAG,
               "UAC MIC: use frame[%u] ch_num = %" PRIu32
               ", bit_resolution = %" PRIu32 ", samples_frequence = %" PRIu32,
               frame_index, s_mic_ch_num, s_mic_bit_resolution,
               s_mic_samples_frequence);
      free(mic_frame_list);
    } else {
    ...
    if (frame_size) {
      ESP_LOGI(TAG, "UAC SPK: get frame list size = %u, current = %u",
               frame_size, frame_index);
      uac_frame_size_t *spk_frame_list =
          (uac_frame_size_t *)malloc(frame_size * sizeof(uac_frame_size_t));
      ...
      for (size_t i = 0; i < frame_size; i++) {
        ESP_LOGI(TAG,
                 ...
                 "= %" PRIu32 ", samples_frequence_min = %" PRIu32
                 ", samples_frequence_max = %" PRIu32,
                 i, spk_frame_list[i].ch_num, spk_frame_list[i].bit_resolution,
                 spk_frame_list[i].samples_frequence,
                 spk_frame_list[i].samples_frequence_min,
                 spk_frame_list[i].samples_frequence_max);
      }
      if (s_spk_samples_frequence !=
              spk_frame_list[frame_index].samples_frequence ||
      ...
      ESP_LOGI(TAG,
               "UAC SPK: use frame[%u] ch_num = %" PRIu32
               ", bit_resolution = %" PRIu32 ", samples_frequence = %" PRIu32,
               frame_index, s_spk_ch_num, s_spk_bit_resolution,
               s_spk_samples_frequence);
      free(spk_frame_list);
    } else {
...
void app_main(void) {
  ...
  uvc_config_t uvc_config = {
      /* match the any resolution of current camera (first frame size as
         default) */
  ...
  xEventGroupWaitBits(s_evt_handle, BIT3_SPK_START, false, false,
                      portMAX_DELAY);

  while (1) {
    xEventGroupWaitBits(s_evt_handle, BIT3_SPK_START, true, false,
                        portMAX_DELAY);
    ...
    const int buffer_size = buffer_ms * (s_spk_bit_resolution / 8) *
                            (s_spk_samples_frequence / 1000);
    ...
    while (1) {
      if ((uint32_t)(s_buffer + offset_size) >=
          (uint32_t)(wave_array_32000_16_1 + s_buffer_size)) {
        ...
        for (size_t i = 0; i < offset_size; i++) {
          d_buffer[i] =
              *(s_buffer + i * freq_offsite_step) >> downsampling_bits;
        }
      ...
      if (xEventGroupGetBits(s_evt_handle) &
          (BIT4_SPK_RESET | BIT3_SPK_START)) {
